...
ipc.on('neurosityEmitter', async (args) => {                                                    // args here only receive one string: the nickname of the device you're getting the info from.
    ...
    notion.calm().subscribe((calm) => {                                                         // Subscribe to the calm score.
        ...
        calmScore = Math.trunc(calm.probability * 100) + "%";                                   // This is a lazy step where I take the calm.probability and format it into a percentage.
        ipc.emit('calmScore', calmScore);                                                       // ...and then send that "x%" back to UE.
    ...
    notion.focus().subscribe((focus) => {                                                       // The exact same thing happens for the focus score.
        ...
        focusScore = Math.trunc(focus.probability * 100) + "%";
        ipc.emit('focusScore', focusScore);
